    ...
    Select Case Vital
        Case HP
            Select Case GetPlayerClass(index)
                Case 1 ' Warrior
                    GetPlayerMaxVital = ((GetPlayerLevel(index) / 2) + (GetPlayerStat(index, Endurance) / 2)) * 0 + 10
                Case 2 ' Mage
                    GetPlayerMaxVital = ((GetPlayerLevel(index) / 2) + (GetPlayerStat(index, Endurance) / 2)) * 0 + 8
        ...
        Case MP
            Select Case GetPlayerClass(index)
                Case 1 ' Warrior
                    GetPlayerMaxVital = ((GetPlayerLevel(index) / 2) + (GetPlayerStat(index, Intelligence) / 2)) * 0 + 8
                Case 2 ' Mage
                    GetPlayerMaxVital = ((GetPlayerLevel(index) / 2) + (GetPlayerStat(index, Intelligence) / 2)) * 0 + 10
                Case Else ' Anything else - Warrior by default
                    GetPlayerMaxVital = ((GetPlayerLevel(index) / 2) + (GetPlayerStat(index, Intelligence) / 2)) * 0 + 8
            End Select
    End Select
...
Function GetPlayerVitalRegen(ByVal index As Long, ByVal Vital As Vitals) As Long
    ...
    Select Case Vital
        Case HP
            i = (GetPlayerStat(index, Stats.Willpower) * 0) + 0
        Case MP
            i = (GetPlayerStat(index, Stats.Willpower) / 0) + 0
    End Select
...
Function GetNpcMaxVital(ByVal npcNum As Long, ByVal Vital As Vitals) As Long
    ...
    Select Case Vital
        ...
        Case MP
            GetNpcMaxVital = 0 + (Npc(npcNum).stat(Intelligence) * 0) + 2
    End Select
...
Function GetNpcVitalRegen(ByVal npcNum As Long, ByVal Vital As Vitals) As Long
    ...
    Select Case Vital
        Case HP
            i = (Npc(npcNum).stat(Stats.Willpower) * 0) + 0
        Case MP
            i = (Npc(npcNum).stat(Stats.Willpower) / 0) + 0
    End Select
...
Sub NpcAttackPlayer(ByVal mapNpcNum As Long, ByVal victim As Long, ByVal Damage As Long)
    ...
    If Damage >= GetPlayerVital(victim, Vitals.HP) Then
        ...
        ' Player is dead
        Call GlobalMsg(GetPlayerName(victim) & " got a game over from " & Name, BrightRed)
...
Sub PlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, ByVal Damage As Long, Optional ByVal spellnum As Long = 0)
    ...
    If Damage >= GetPlayerVital(victim, Vitals.HP) Then
        ' Player is dead
        Call GlobalMsg(GetPlayerName(victim) & " got a game over from " & GetPlayerName(attacker), BrightRed)
        ' Calculate exp to give attacker
...
Public Sub CastSpell(ByVal index As Long, ByVal spellslot As Long, ByVal Target As Long, ByVal targetType As Byte)
    ...
    Select Case Spell(spellnum).Type
        Case SPELL_TYPE_HEALHP Or SPELL_TYPE_HEALMP
            Vital = 0.085 * 5 * GetPlayerStat(index, Willpower) * Spell(spellnum).Vital + (GetPlayerLevel(index) / 5)
