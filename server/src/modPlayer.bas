...
Sub JoinGame(ByVal index As Long)
    ...
    Else
        Call GlobalMsg(GetPlayerName(index) & " has joined " & Options.Game_Name & "!", Blue)
    End If
...
Function GetPlayerNextLevel(ByVal index As Long) As Long
    GetPlayerNextLevel = (20 / 3) * ((GetPlayerLevel(index) + 1) ^ 3 - (6 * (GetPlayerLevel(index) + 1) ^ 2) + 16 * (GetPlayerLevel(index) + 1) - 12)
End Function
